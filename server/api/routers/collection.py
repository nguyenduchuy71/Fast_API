...
from db import crud
from schemas import item, collection
...
def share_collection(friendId: str, item:collection.CollectionShare, db: Session = Depends(get_db), current_user = Depends(oauth2.get_current_user)):
    try:
        db_user = crud.get_user(db, user_id=current_user.id)
        if db_user is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User not found")
        crud.share_friend_item(db, current_user.id, friendId, item.srcImage)
